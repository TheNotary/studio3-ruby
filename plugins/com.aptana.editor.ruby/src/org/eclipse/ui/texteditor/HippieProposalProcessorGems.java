 ...
 * @since 3.2
 */
public final class HippieProposalProcessorGems implements IContentAssistProcessor {
	 ...
	 */
	public HippieProposalProcessorGems() {
	}
	...
	private List getSuggestions(ITextViewer viewer, int offset, String prefix) throws BadLocationException {
		...
		// FIXME: My hacks for adding arbitrary files to the sniffup pool...
		//IDocument myDocument = new SynchronizableDocument();
		//myDocument.set("def find_this_method_please; end;");
		
		ArrayList suggestions= createSuggestionsFromOpenDocument(viewer, offset, prefix);
		IDocument currentDocument= viewer.getDocument();
		
		String[] requiredFiles = MyGemFunctions.parseCurrentFileForRequires();
		String[] requiredFilesPaths = MyGemFunctions.locateSourceCodeOfRequiredFiles(requiredFiles);
		
		IDocument sourceCodesToParse[]= MyGemFunctions.pullUpSourceFilesAsIDocuments(requiredFilesPaths);
		
		for (int i= 0; i < sourceCodesToParse.length; i++) {
			IDocument doc = sourceCodesToParse[i];
